...
- 2026-09-17 (autonomous-fix session) — **Cloud Run port bug fixed (was wiping instance state)**: serve bound the 8477 default while Cloud Run startup-probes PORT=8080, so every new instance failed readiness and was replaced within minutes — each replacement wiped the ephemeral DB (this explains the recurring "services_configured: false after deploy" surprise). Fix: port precedence WAKEY_PORT > PORT > 8477 (config), Cloud Run template pins WAKEY_PORT=8080 (gcp_deploy), Dockerfile healthcheck follows the same resolution. Deployed rev 00016 verified stable on 8080.

- 2026-09-17 (autonomous-fix session, live verify) — **Unattended chain ran live with zero human actions**: fixture registered (autonomy=fix, floor 0.5) → 3 tracebacks ingested → ticket auto-created (linux-clipboard-manager #11) → GLM RCA posted `code-fix` 0.85 → **auto-fix trigger fired the fix executor on its own** (the new 441087a wiring): workspace fetched, 4 model/patch/sandbox-test iterations cycled. Final draft PR did not land within the session window: the 4th sandboxed test run on Cloud Run's degraded path (no network namespace — warning logged per run) wedged past the ~5 min/run pattern of earlier runs; RCA background loop blocks while the fix runs (by design, serialized). OPEN (M3 hardening): enforce per-command timeout on the degraded sandbox path + add a fix-loop watchdog so a wedged test run can't hold the RCA loop; stale FIXING recovery on instance restart.
...
- 2026-09-17 (defect-closure session) — **All five reproduced v0.1.0 review defects fixed with pinned regression tests** (tests/unit/test_reliability_regressions.py): (1) `/webhooks/github` now verifies native GitHub signatures — `X-Hub-Signature-256` against the connected App's stored webhook secret (decrypts via SecretBox) or `WAKEY_WEBHOOK_SECRET` — and dedups on `X-GitHub-Delivery`; a normally signed GitHub request no longer 401s. (2) Delivery retries count exactly once: `Storage.claim_event` persists the event, counts the occurrence, and sets a `counted` flag in ONE transaction (schema v6, SQLite + Postgres); dispatch is journaled after the policy step, so a forge failure replays the policy on retry (state machine absorbs) without re-counting — 3 events now produce exactly 3 occurrences and 1 ticket even across an outage. (3) Crash-interrupted ticket creation self-heals: fingerprints reserve with `reserved_at`; a stale reservation (QUEUED_RCA, no ticket, >120s) re-arms the wake path instead of staying ticketless forever. (4) Verification binds environment + fix commit + deploy-relative window: deploy webhook records the payload's environment; `build_grace_inputs` matches deploys on (service, fingerprint environment, fix sha bound at merge from `merge_commit_sha`) and opens the grace window at fix-deploy time — a staging deploy can never mark a prod fingerprint verified. (5) `snapshot_files` never reads through symlinks (files or dirs). Also: **GitHub App installation-token auth completes the operational connection** — `sign_app_jwt` (RS256), `InstallationTokenSource` (mint + cache + pre-expiry refresh), adapter `token_provider`, App-only onboarding builds the live forge without a PAT (forge simulator gained the /app/installations endpoints); auto_fix RCA→fix dispatch verified wired through composition. Fixed en route: Postgres `list_active_fingerprints` used SQLite `?` placeholders (would crash RCA/watcher loops on OPS-5). make check now enforces the CI coverage floor locally (aligning with AGENTS.md) and lints demo/. Gates: 344 tests + 8 sandbox-gated, coverage 90.4%, ruff/mypy-strict/SPDX green, PG suite green against live Postgres. Still open for v1.0: live-GitHub + container-gate reruns, P1 scope items (@wakey retry w/ reviewer feedback, digest/approval mode, independent diff-vs-RCA verification, deploy-diff RCA, tiered routing + spend accounting, docker/journald sidecar), credential rotation.
