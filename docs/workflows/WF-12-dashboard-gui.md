...
| 3 | **Fingerprints & live board** | **The live task board** (OPS-11): kanban columns = WorkStates (`new / investigating / awaiting-human / fixing / verifying / done`), cards = fingerprints with service, ×count, age, owner, agent activity ("RCA running · 2m"); click-through to detail view: occurrence chart (deploy markers overlaid), event samples (redacted), linked ticket/PR with GitHub links, RCA summary + confidence meter + cost chips, suppression controls (suppress with duration + reason — same write path as `@wakey drop`/DET-7, audited). **Manual fix trigger** (founder directive 2026-09-16): a "request fix attempt" button on the detail page runs the exact FIX-1 eligibility gate (class, confidence floor, autonomy dial, PR cap, chronic guard), surfaces the named outcome as a flash, and audits `fix.requested` either way — human-initiated, never auto-triggered, never merges (executor lands with E7-T1/E8). Basic board (aggregated, on-load) in P1; live updates via SSE in P2. Also table view with filters (service, severity, state, age). |
...
- **Auth**: session cookie (HttpOnly/Secure/SameSite per WF-01); login = first-run token (once) then local admin session; P2 adds GitHub-identity RBAC. Remote access over HTTPS is the operator's reverse-proxy responsibility — documented, with a warning banner when `BASE_URL` isn't HTTPS. *Implemented 2026-09-16 (R-07 closure):* `wakey/web/auth.py` + `register_auth_routes` — setup token single-use/30-min/regenerable via `wakey setup-token --force`; admin sessions server-side with HttpOnly+SameSite=Lax cookies (Secure when BASE_URL is HTTPS); `/board`, `/settings`, `/api/settings` require a session; `/healthz`, `/readyz`, `/metrics` and ingest/webhooks stay intentionally public (aggregate counters + key-verified writes). The serve banner prints the first-run token.
...
- **The one dashboard action on incidents** is the human-initiated fix request (WF-12 page 3b): it dispatches nothing on its own — it runs the shared FIX-1 gate and records the outcome. Review and merge stay in GitHub.
