...
7. **Capacity & scale** (`NFR-2`): single-box target 25 services / 50 repos / 5M events/day (SQLite); beyond → Postgres path (`OPS-5`); queue depth + ingest latency are the scaling signals (documented thresholds); horizontal ingest replicas is a P3 concern (`OPS-10`), but the queue interface is built replica-ready from E2-T5. **Downward too** (`NFR-6..8`): resource budgets are CI-enforced (idle ≤120MB, steady-state RSS ≤350MB, disk ≤25MB/day, boot <5s) and `resource_profile: lean` targets Raspberry-Pi/small-VPS class hosts — the capacity story runs in both directions.
