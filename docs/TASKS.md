...
| E1-T2 | CI pipeline: PR gates (lint→type→unit→contract→security→e2e-fake→image build), nightly slot reserved for eval bench | — | eng-standards §4 | PR without green CI cannot merge; pipeline <10min; image builds with SBOM | E1-T1 | M | in-progress (workflow committed; nightly/eval/SBOM slots land with later tasks; execution pending remote) |
| E1-T3 | Docker packaging: Dockerfile (non-root, slim), docker-compose dev + prod profiles, healthchecks | ONB-1 | WF-11 | `docker compose up` starts wakeyd+DB, `/healthz` green, container runs as non-root | E1-T2 | S | in-progress (image builds/runs at 149MB non-root ✓; `/healthz` AC lands with the E2-T4 server) |
