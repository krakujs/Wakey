...
from wakey.ingest.normalizers import map_severity, parse_events
...
def test_jsonl_lines_become_structured_events() -> None:
    ...
    outcome = parse_events(raw, service="api", environment="prod", source="webhook")
...
def test_logfmt_detected_and_severity_mapped() -> None:
    ...
    outcome = parse_events(raw, service="api", environment="prod", source="docker")
...
def test_empty_lines_skipped_and_ids_stable() -> None:
    ...
    outcome = parse_events(raw, service="api", environment="prod", source="webhook")
    assert len(outcome.events) == 2
    assert outcome.events[0].id == outcome.events[1].id  # deterministic replay ids
